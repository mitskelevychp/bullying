...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Статті за ${year} рік`,
    description: `Перелік усіх статей, опублікованих у ${year} році.`,
  };
}

export async function generateStaticParams() {
  const years = getAllYearsForStaticParams(); // Використовуємо нову функцію
  return years;
}

export default async function YearPage({ params }: Props) {
  const { year } = await params;
  const posts: PostMetadata[] = getPostsByYear(year);
